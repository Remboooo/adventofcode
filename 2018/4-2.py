...
def process(data):
    ...
    max_asleep_guard = [None] * 60
    max_asleep_counts = [0] * 60
    ...
    for guard, shifts in guard_shifts.items():
        ...
        for i, count in enumerate(minute_asleep_counts):
            if count > max_asleep_counts[i]:
                max_asleep_counts[i] = count
                max_asleep_guard[i] = guard

    max_minute = max_asleep_counts.index(max(max_asleep_counts))
    max_guard = max_asleep_guard[max_minute]

    print("Max: #{} at minute {}; mult = {}".format(max_guard, max_minute, max_guard * max_minute))
